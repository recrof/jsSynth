 ...
 * Copyright 2012-14, Stuart Memo, forcer
 *
...
(function(window, undefined) {
    var QwertyHancock = function(settings) {
        ...
        var midiSuccess = function(access) {
            ...
            console.log('Web MIDI API initialized, midi inputs:', access.inputs());
            midi_input.onmidimessage = function(event) {
                var data = event.data;
                var key_pressed = midi_notes[data[1] % 12] + parseInt(data[1] / 12);

                console.info(data[0].toString(16), data[1].toString(16), data[2].toString(16));
        ...
        var midiError = function(err) {
            console.error("Midi access restricted(maybe you forgot to enable it in chrome://flags): " + err.code);
        };
